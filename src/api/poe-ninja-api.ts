...
export class PoeNinjaClient {
  ...
  async fetchAllMarketData(): Promise<{
    ...
    console.log('Fetching market data from poe.ninja...');
    ...
    console.log(`Market data loaded: Divine = ${divinePrice}c`);
    console.log(`  Unique items: ${uniques.lines.length}`);
    console.log(`  Skill gems: ${gems.lines.length}`);
    console.log(`  Currency: ${currency.lines.length}`);
    console.log(`  Fragments: ${fragments.lines.length}`);
    console.log(`  Divination cards: ${divination.lines.length}`);
  ...
  private async fetchItemOverview(type: string): Promise<PoeNinjaResponse> {
    try {
      ...
      console.warn(`Failed to fetch ${type} from poe.ninja:`, error);
      return { lines: [] };
    }
  }
